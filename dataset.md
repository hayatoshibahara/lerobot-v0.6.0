...
3. データセットの名前、公開範囲、ローカル保存先、タスク説明を決める。
...
この資料のコマンドはデータセットリポジトリを **公開（Public）** で作成する。公開リポジトリでは映像を含むデータが誰でも閲覧・ダウンロードできる。収録前に作業台の周囲から人物、顔、個人情報、社外秘の物、著作権上共有できない物を片付ける。それらを取り除けない場合は、`--dataset.private=true` を追加して非公開で収録する。
...
この後の `lerobot-record` に `--dataset.repo_id="$DATASET_REPO_ID"` を渡すと、アップロード時に同名の **データセットリポジトリ** が公開（Public）で自動作成される。通常は Hub の画面で事前作成する必要はない。本資料ではこの自動作成を使う。

収録前にリポジトリを手動作成したい場合だけ、空のリポジトリを作る。

1. Hub のプロフィールメニューから **New Dataset** を選び、`DATASET_NAME` と同じ名前を指定する。
2. 公開してよい内容なら **Public**、人物や個人情報が映る可能性があるなら **Private** を選び、作成する。
...
非公開にする場合は `--private` を追加する。どちらの方法でも、後述する `lerobot-record` は同じリポジトリへアップロードする。
...
このコマンドは `DATASET_REPO_ID` を公開リポジトリとして自動作成する。人物や個人情報が映る可能性を排除できない場合は、末尾に `--dataset.private=true` を追加して非公開で収録する。
...
ライセンスは例である。データの権利関係に合わせて選び、判断できない場合は `--dataset.private=true` を使い非公開にする。収録前にカメラの画角を確認し、人物、名札、画面、書類、社外秘の物、第三者の著作物が映らないようにする。公開リポジトリはアップロードと同時に誰でも閲覧できるため、アップロード後は速やかに Hub の Visualizer で全映像を見直し、問題が見つかった場合は直ちにリポジトリを非公開に変更するか削除する。
